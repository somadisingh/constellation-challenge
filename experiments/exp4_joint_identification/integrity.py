"""Protected-artifact hashing for Experiment 4 (rule 5, 6).

Broader than Experiment 3's own `integrity.py`: Experiment 4 must additionally
never modify Experiment 3's own outputs/checkpoints, since Exp4 reads them
read-only to build the leak-free fixed-arm-F evaluation.
"""
...
def _iter_files(root: Path):
    ...
    top-level Markdown documentation (`*.md`). Documentation under `lab/` (e.g.
    `lab/LEDGER.md`) is explicitly meant to be updated by every experiment's own
    final-write step -- the same way top-level README.md/FINDINGS.md are never
    in `PROTECTED_ROOTS` at all -- so it must not be treated as a protected
