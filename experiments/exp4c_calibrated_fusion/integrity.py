"""Protected-artifact hashing for Experiment 4C. Broader than Experiment 4B's
own list: 4C must additionally never modify Experiment 4B's own outputs,
since 4C reads them read-only (frozen hypothesis pools, gates, metrics) to
ground its calibrated-fusion repair.
"""
...
def _iter_files(root: Path):
    ...
    Markdown documentation (updated by every experiment's final write step,
    like top-level README.md/FINDINGS.md, which are never protected at all)."""
    if root.is_file():
